...
def test_bootstrap_expired_plan(mock_db, mock_claims):
    ...
    # Mock Expired Subscription (get_active_subscription returns None)
    mock_db.query.return_value.filter.return_value.order_by.return_value.first.return_value = None
...
def test_bootstrap_device_mismatch(mock_db, mock_claims):
    """
    Valida que si el device_id del token no coincide con el de la DB (User),
    se devuelva 409 Conflict.
    """
    override_deps(mock_db, mock_claims)
    
    # User tiene un device_id diferente al del token (mock_claims tiene "device_test_100")
    user = User(id=100, email="test@example.com", device_id="device_other_200")
    mock_db.query.return_value.filter.return_value.first.return_value = user
